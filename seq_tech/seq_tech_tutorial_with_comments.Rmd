---
title: "7BBG1002: Sequencing Technologies Tutorial"
...
Run bowtie2 to align sequencing reads:

bowtie2 -x results/exercise1/bowtie_index/varicella \
        -1 data/seq/varicella1.fastq -2 data/seq/varicella2.fastq \
        -S results/exercise1/alignment/varicella.sam
This command runs the Bowtie2 aligner to map paired-end sequencing reads to a reference genome. Let's break down the command options:

-x results/exercise1/bowtie_index/varicella: Specifies the path to the Bowtie2 index for the reference genome. In this case, it is using an index located at results/exercise1/bowtie_index/varicella.

-1 data/seq/varicella1.fastq: Specifies the first FASTQ file containing the forward reads (read 1) from a paired-end sequencing run.

-2 data/seq/varicella2.fastq: Specifies the second FASTQ file containing the reverse reads (read 2) from a paired-end sequencing run.

-S results/exercise1/alignment/varicella.sam: Indicates the output file format and location. The -S flag tells Bowtie2 to generate a SAM file, which is a standard format for storing aligned sequences. The output will be saved as varicella.sam in the results/exercise1/alignment directory.

Overall Purpose:
The entire set of commands aligns the paired-end sequencing reads from the input files (varicella1.fastq and varicella2.fastq) to the reference genome index located in results/exercise1/bowtie_index/varicella. The resulting alignments are saved in the varicella.sam file.

This process is typically used to identify where the sequencing reads map to the reference genome, which is an essential step in downstream genomic analyses, such as variant calling or gene expression profiling.
...
samtools view: This is the samtools command for viewing and converting SAM/BAM files.

-b: This flag tells samtools to output the file in BAM format (binary version of the SAM format). BAM files are compressed and take up less space than SAM files.

-S: This flag specifies that the input file is in SAM format. (Note: This option is typically not needed in newer versions of samtools if the input file has a .sam extension.)

-o results/exercise1/alignment/varicella.bam: The -o option specifies the output file name. In this case, the converted BAM file will be saved as varicella.bam in the results/exercise1/alignment/ directory.

results/exercise1/alignment/varicella.sam: This is the input file in SAM format (from the previous bowtie2 alignment step).

Overall Purpose:
This command converts the varicella.sam file (a text-based format) into a varicella.bam file (a more compact binary format), making it easier to handle for subsequent processing steps. BAM files are the preferred format for storing large-scale alignment data due to their smaller size and efficient random access capability.

Why Use BAM Instead of SAM?
Space Efficiency: BAM is compressed, which significantly reduces storage requirements compared to SAM.
Faster Access and Processing: Most bioinformatics tools can process BAM files more quickly and efficiently.
Indexing Compatibility: BAM files can be indexed, allowing for fast retrieval of reads from specific genomic regions.
...
|--------|----------------------------------------------------------------------------------------------|-------------------------------------|
| 1      | **Chromosome or scaffold name**: The name of the reference chromosome or contig               | `NC_001348`                         |
| 2      | **Position**: The genomic position (1-based) in the reference genome                          | `73233`                             |
| 3      | **Reference base**: The base at this position in the reference genome                         | `t` (lowercase, indicating thymine) |
| 4      | **Read depth**: The number of reads that cover this position                                  | `90`                                |
| 5      | **Read bases**: Encodes observed bases at this position in aligned reads, along with metadata | `,,,.,.g,,,,,,..,,.,,.,,..,..,A,...` |
| 6      | **Base quality scores**: ASCII-encoded quality scores for the observed bases                  | `222C2C22222222222222C22222C222...` |

#### Understanding the Read Bases Column (Column 5)

The read bases column (`,,,.,.g,,,,,,..,,.,,.,,..,..,A,.,.,.,..,.,,..,..,,g..,.,...,...,.,,,,.....,,...,,,.,.,,..^K.`) encodes the bases observed at this position and additional information:

- **`.` or `,`**: Bases that **match the reference base** (`t`).
  - `.`: Match on the **forward strand**.
  - `,`: Match on the **reverse strand**.
- **`A`, `C`, `G`, `T`**: Bases that **differ from the reference** (alternate alleles).
  - Uppercase: Observed on the **forward strand**.
  - Lowercase: Observed on the **reverse strand**.
- **`^` followed by a character** (e.g., `^K`): **Start of a read**. The following character represents the mapping quality.
- **`$`**: **End of a read**.
- **Insertions and deletions**:
  - `+<number><bases>`: Insertion relative to the reference.
  - `-<number><bases>`: Deletion relative to the reference.

#### Interpreting the Base Quality Scores (Column 6)

The base quality scores (`222C2C22222222222222C22222C222...`) are **ASCII-encoded Phred scores** representing the probability of base call errors:

- Phred Score (`Q`) = `-10 * log10(probability of error)`
- **ASCII to Phred Conversion**:
  - `2` in ASCII = 50, so `50 - 33 = 17` (Phred score 17).
  - `C` in ASCII = 67, so `67 - 33 = 34` (Phred score 34).

#### Example Observations:
1. **Position:** `73233` on chromosome `NC_001348`.
2. **Reference Base:** `t` (thymine).
3. **Read Depth:** `90` reads.
4. **Base Composition:**
   - Most reads match the reference (`.` or `,`).
   - A few alternate alleles: `g` (on reverse strand) and `A` (on forward strand).
5. **Quality Scores:** Most bases have scores of `17` or `34`, indicating **high confidence**.

#### Final Interpretation
This pileup output suggests a high-confidence base at position `73233` with a few observed alternate alleles (`A` and `g`). This may indicate a potential SNP. Further analysis of the alternate allele frequency and quality is required to confirm if this is a true variant.
