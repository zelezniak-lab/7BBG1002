...
Before we begin please, login to CREATE and copy the following script to your home directory.
The file is a running script to start Rstudio server that you will need to login.

```bash
cp /scratch/grp/msc_appbio/ops-rstudio.sh .

```

From you home directory run the following command.

```bash
sbatch ops-rstudio.sh
```

Follow instructions in slurm output file by reading it using `cat slurm-xxxxxxxx.out` where xxxxxxxx is submitted job id. 
